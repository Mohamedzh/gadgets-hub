...
export const getLatestReviews = async () => {
    ...
    const api =
        wretch("https://www.gsmarena.com/", { mode: "cors" })
            .errorType("text")
            .resolve(r => r)
    ...
    reviewsTag.each((i, el) => {
        ...
        if (brands.find(brand => brand.name === extractedBrand)) {
            ...
            if (extractedBrand === 'poco') {
                brandName = 'xiaomi'
            } else
                if (extractedBrand === 'iqoo') {
                    brandName = 'vivo'
                } else if (extractedBrand === 'nubia') {
                    brandName = 'zte'
                }
        (link && imgUrl &&
            newReviews.push({ title, link, imgUrl, reviewDate, brandName }))
    })
    return newReviews
}
    ...
    for (let i = 0; i < urls.length; i++) {
        ...
        setTimeout(async () => {
            ...
            if (data.pages.length > 0) {
                ...
                phones.each((i, el) => {
                    const imgBlock = $(el).find('img')
                    const yearDesc = imgBlock.attr('title')?.slice(0, imgBlock.attr('title')?.indexOf('Features'))
                    const phone = {
                        name: $(el).find('span').text(),
                        imgUrl: imgBlock.attr('src')!,
                        url: $(el).find('a').attr('href')!.replace('.php', ''),
                        description: imgBlock.attr('title')!,
                        year: Number(yearDesc!.slice(yearDesc?.indexOf('20', 20), yearDesc?.indexOf('20', 20)! + 4)),
                    }
                    if (phone.year > 2018) {
                        json.push(phone)
                    }
