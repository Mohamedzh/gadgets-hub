...
export default function ProfileSection({ user }: Props) {
    ...
    const userData = [
        { name: 'Nickname', arabicName: 'الاسم', value: user.user_metadata.nickName, placeHolder: 'Your new Nickname', arabicPlaceholder: 'الاسم الجديد', disabled: false },
        { name: 'Email Address', arabicName: 'البريد الالكتروني', value: user.email, placeHolder: 'Your new email address', arabicPlaceholder: 'البريد الالكتروني الجديد', disabled: false },
        { name: 'Password', arabicName: 'كلمة السر', value: '******', placeHolder: 'Your new password', arabicPlaceholder: 'كلمة السر الجديدة', disabled: false },
    ]
    ...
    return (
        <>
            ...
            <div className="mt-5 border-t border-gray-200">
                <dl className="divide-y divide-gray-200">
                    {userData.map((data, i) =>
                        <div key={i} className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-5">
                            <dt className=" font-medium text-green-500">{arLang ? data.arabicName : data.name}</dt>
                                        ...
                                        <div className="mt-1 flex rounded-md shadow-sm">
                                            <div className="relative flex flex-grow items-stretch focus-within:z-10">
                                                ...
                                                <input
                                                    ...
                                                    name="email"
                                                    id="email"
                                                    className={`block w-full text-black rounded-none ${arLang ? 'rounded-r-md' : 'rounded-l-md'} border-gray-300 pl-10 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm`}
                                                    placeholder={arLang ? data.arabicPlaceholder : data.placeHolder}
                                                />
                                            </div>
                                            <button
                                                onClick={() => {
                                                ...
                                                type="button"
                                                className={`relative -ml-px inline-flex items-center space-x-2 ${arLang ? 'rounded-l-md' : 'rounded-r-md'} border border-gray-300 bg-gray-50 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500`}
                                            >
                                                {/* <BarsArrowUpIcon className="h-5 w-5 text-gray-400" aria-hidden="true" /> */}
                                                <span>{arLang ? 'حفظ التغييرات' : 'Save changes'}</span>
                                            </button>
                                        </div>
                                    </div>
                                }

                                <span className={`flex-shrink-0 ${arLang ? 'mr-auto' : 'ml-auto'}`}>
                                    <button
                ...
                <ProfileFavsSection user={user} arLang={arLang} />
            </div>
        </>
    )
}
