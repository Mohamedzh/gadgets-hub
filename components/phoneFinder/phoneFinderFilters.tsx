...
import {
  ...
  clearActiveFilters,
  removeFromFilters,
} from "../../redux/slices/activeFiltersSlice";
import {
  addToRam,
  clearRamFilter,
  removeFromRam,
} from "../../redux/slices/ramFilterSlice";
import {
  addToDisplaySize,
  clearDisplaySizeFilter,
  removeFromDisplaySize,
} from "../../redux/slices/displaySizeFilterSlice";
import {
  addToBatterySize,
  clearBatterySizeFilter,
  removeFromBatterySize,
} from "../../redux/slices/batterySizeFilterSlice";
...
export default function Filters({ quickSpecs, brands, phones }: Props) {
  ...
  const setFilterChoices = (sectionName: string, option: string) => {
    if (sectionName === "ramSize") {
      if (filterRam.includes(option)) {
        dispatch(removeFromRam(option));
      } else {
        dispatch(addToRam(option));
      }
    } else if (sectionName === "displaySize") {
      if (filterDisplaySize.includes(option)) {
        dispatch(removeFromDisplaySize(option));
      } else {
        dispatch(addToDisplaySize(option));
      }
    } else if (sectionName === "batterySize") {
      if (filterBatterySize.includes(option)) {
        dispatch(removeFromBatterySize(option));
      } else {
  ...
  for (let i = 0; i < phones.length; i++) {
    phones[i].PhoneQuickSpecs.map((quickSpec) => {
      if (
        quickSpec.quickspecName === "RAM size" &&
        quickSpec.value.length > 0
      ) {
        quickSpec.value = convertRAM(quickSpec.value);
      } else if (
        quickSpec.quickspecName === "Display size" &&
        quickSpec.value.length > 0
      ) {
        quickSpec.value = convertDisplaySize(quickSpec.value);
      } else if (
        quickSpec.quickspecName === "Battery size" &&
        quickSpec.value.length > 0
      ) {
        quickSpec.value = convertBatterySize(quickSpec.value);
      } else if (
        quickSpec.quickspecName === "Release date" &&
        quickSpec.value &&
        quickSpec.value.length > 0
      ) {
        quickSpec.value = convertDate(quickSpec.value);
      } else if (
        quickSpec.quickspecName === "OS" &&
        quickSpec.value.length > 0
      ) {
  ...
  let uniquesSpecs: { name: string; options: string[] }[] = uniqueSpecNames.map(
    (item) => {
    ...
    <div className="">
      ...
      <div>
        <div className="flex flex-col place-content-center place-items-center">
          ...
          >
            {t("filtersSearchNowButton")}
          </button>
        </div>
        {activeFilters.length > 0 ? (
          <PhoneResultList currentPhones={currentPhones} />
        ) : (
          <p className="text-white text-center m-5">{t("filtersSubHeader")}</p>
        )}
      </div>
    </div>
  );
