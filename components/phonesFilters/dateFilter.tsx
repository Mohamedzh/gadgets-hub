...
export default function RangeSlider({
  ...
  const setYearFilter = (newValue: number[]) => {
    setSortedPhones(
      phones.filter(
        (phone) => phone.year >= newValue[0] && phone.year <= newValue[1]
      )
    );
  };
  ...
  return (
    <ThemeProvider theme={theme}>
      <Box sx={{ width: { xs: 100, sm: 150, md: 200, lg: 275 } }}>
        <p className="lg:mb-4 lg:mt-10 text-sm lg:text-xl font-semibold text-gray-200">
          {t("filterByYear")}
        </p>
        <Slider
          sx={{ color: "blue" }}
