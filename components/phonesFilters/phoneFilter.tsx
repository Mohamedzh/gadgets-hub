...
import { classNames, englishLocale, paginate } from "../../lib/functions";
...
let sortOptions = [
  { name: "filtersA-Z", current: false },
  { name: "filtersZ-A", current: false },
  { name: "filtersMostPopular", current: true },
...
export default function PhoneFilter({
  ...
  useEffect(() => {
    setPageNo(Math.ceil(sortedPhones.length / 40));
  }, [sortedPhones]);
  ...
  const setActiveFilter = (
    sortingOptions: { name: string; current: boolean }[],
    option: { name: string; current: boolean }
  ) => {
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-baseline justify-between border-b border-gray-200 pt-24 pb-6">
            <h1 className="text-4xl font-bold text-blue-300">
              {englishLocale(router) ? brand.toUpperCase() : arabicBrandName}
            </h1>
              ...
              <Menu as="div" className="relative inline-block text-left">
                <div>
                  <Menu.Button className="group inline-flex justify-center text-sm font-medium text-gray-100 hover:text-gray-400">
                    <ChevronDownIcon
                      className={`${
                        englishLocale(router) ? "ml-1" : "mr-1"
